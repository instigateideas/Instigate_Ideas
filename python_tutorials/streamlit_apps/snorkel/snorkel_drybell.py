...
# Max length to be tokenized - 90 Percentile
st.sidebar.header("Max Length - BERT Tokenisation")
percentile_value = st.sidebar.text_input("Percentile Value", 90)
...
def get_clf_objects(params):
	for clf_name, v in params.items():
		if clf_name == "Logistic Regression":
			max_iter = int(params[clf_name]["param"]["max_iter"])
			random_state = int(params[clf_name]["param"]["random_state"])
			clf = LogisticRegression(max_iter=max_iter, random_state=random_state)
			params[clf_name]["clf_obj"] = clf
		elif clf_name == "Decision Tree":
			st.sidebar.write("Decision Tree params goes here..")
		elif clf_name == "Random Forest":
			n_estimators = int(params[clf_name]["param"]["n_estimators"])
			criterion = params[clf_name]["param"]["criterion"]
...
clf_params = get_clf_objects(params)
st.write(clf_params)
